...
export function OAuthButtons() {
  ...
  return (
    <div className="flex flex-col gap-3">
      {PROVIDERS.map(({ id, label, svg }) => (
        <button
          key={id}
          type="button"
          onClick={() => void start(id)}
          disabled={busy !== null}
          className="flex h-auto cursor-pointer items-center justify-center gap-2.5 rounded-none border-2 border-ink bg-[#fdfdfb] px-[18px] py-3 text-[15px] font-bold tracking-[0.06em] shadow-[3px_3px_0_0_var(--ink)] hover:bg-ink hover:text-paper disabled:pointer-events-auto disabled:cursor-progress disabled:opacity-60"
        >
          {svg}
          {busy === id ? "STARTING…" : `sign in with ${label.toLowerCase()}`}
        </button>
      ))}
